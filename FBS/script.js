        ...
        function checkAdminStatus() {
            ...
            if (isAdmin) {
                ...
                if (!historyLoaded) loadOrderHistory();
                else renderHistory();
            } else {
        ...
        async function loadOrderHistory() {
            ...
            if (list) {
                list.innerHTML = `<div class=\"bg-white rounded-2xl p-10 text-center text-gray-400 border border-gray-100\"><i class=\"fas fa-circle-notch fa-spin text-2xl mb-3 text-orange-500\"></i><p>Loading orders...</p></div>`;
            }
            try {
                const response = await fetch('/api/orders', { credentials: 'include' });
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.message || 'Could not load order history.');
                orderHistory = Array.isArray(data.orders) ? data.orders : [];
                historyLoaded = true;
                try {
                    localStorage.setItem('fastbite_pos_order_history', JSON.stringify(orderHistory));
                } catch(e) {}
                ...
                // Keep old browser records visible if the API is temporarily unavailable.
                try {
                    const cached = JSON.parse(localStorage.getItem('fastbite_pos_order_history') || '[]');
                    orderHistory = Array.isArray(cached) ? cached : [];
                } catch(e) { orderHistory = []; }
                const listEl = document.getElementById('history-list');
                if (listEl) {
                    listEl.innerHTML = `<div class=\"bg-white rounded-2xl p-10 text-center text-red-400 border border-red-100\"><i class=\"fas fa-triangle-exclamation text-2xl mb-3\"></i><p class=\"font-bold text-gray-700\">Unable to load live history</p><p class=\"text-xs mt-1\">${escapeHtml(error.message)}</p></div>`;
                }
            } finally {
        ...
        function toggleOrderDetails(index) {
            ...
            const icon = document.getElementById(`history-chevron-${index}`);
            if (!details) return;
        ...
        function renderHistory() {
            ...
            const filtered = orderHistory.filter(o =>
                String(o.billNo || '').toLowerCase().includes(query) ||
                String(o.customerName || '').toLowerCase().includes(query) ||
                String(o.mobile || '').includes(query) ||
                String(o.paymentMethod || '').toLowerCase().includes(query)
            );
            ...
            list.innerHTML = filtered.map((order, index) => {
                ...
                const safeItems = items.map(item => `
                    <div class=\"history-item-row\">
                        <div class=\"min-w-0\"><p class=\"font-semibold text-gray-800 truncate\">${escapeHtml(item.name)}</p><p class=\"text-[11px] text-gray-400\">₹${Number(item.price || 0).toFixed(2)} × ${Number(item.qty || 0)}</p></div>
                        <span class=\"font-bold text-gray-800 shrink-0\">₹${(Number(item.price || 0) * Number(item.qty || 0)).toFixed(2)}</span>
                    </div>`).join('');

                return `
                    <div class=\"bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden\">
                        <button type=\"button\" onclick=\"toggleOrderDetails(${index})\" class=\"w-full p-5 md:p-6 flex items-center justify-between gap-4 text-left hover:bg-gray-50 transition-colors\">
                            <div class=\"min-w-0 flex-1\">
                                <div class=\"flex items-center gap-2 flex-wrap\">
                                    <span class=\"font-bold text-gray-800 text-base md:text-lg\">#${escapeHtml(order.billNo)}</span>
                                    <span class=\"text-[10px] bg-green-100 text-green-700 px-2 py-1 rounded font-bold uppercase\">${escapeHtml(order.paymentMethod || '—')}</span>
                                </div>
                                <div class=\"flex flex-wrap gap-x-4 gap-y-1 mt-2 text-sm text-gray-600\">
                                    <span><i class=\"fas fa-user mr-1.5 text-gray-400\"></i>${escapeHtml(order.customerName || 'Guest')}</span>
                                    <span><i class=\"fas fa-phone mr-1.5 text-gray-400\"></i>${escapeHtml(order.mobile || '—')}</span>
                                </div>
                            </div>
                            <div class=\"text-right shrink-0\">
                                <p class=\"text-[10px] text-gray-400 uppercase font-bold\">Total</p>
                                <p class=\"text-lg md:text-xl font-bold text-orange-600\">₹${Number(order.total || 0).toFixed(2)}</p>
                                <i id=\"history-chevron-${index}\" class=\"fas fa-chevron-down text-gray-400 text-xs mt-1 transition-transform duration-200\"></i>
                            </div>
                        </button>
                        <div id=\"history-details-${index}\" class=\"hidden border-t border-gray-100 bg-gray-50/70 p-5 md:p-6\">
                            <div class=\"grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-5\">
                                <div class=\"history-info-box\"><span>Customer</span><strong>${escapeHtml(order.customerName || 'Guest')}</strong></div>
                                <div class=\"history-info-box\"><span>Mobile</span><strong>${escapeHtml(order.mobile || '—')}</strong></div>
                                <div class=\"history-info-box\"><span>Payment</span><strong>${escapeHtml(order.paymentMethod || '—')}</strong></div>
                                <div class=\"history-info-box\"><span>Date & Time</span><strong>${escapeHtml(order.timestamp || '—')}</strong></div>
                            </div>
                            <h3 class=\"text-xs font-bold uppercase tracking-wider text-gray-400 mb-2\">Ordered Items</h3>
                            <div class=\"bg-white rounded-xl border border-gray-100 overflow-hidden\">${safeItems || '<p class=\"p-4 text-sm text-gray-400\">No item details available.</p>'}</div>
                            <div class=\"mt-4 ml-auto max-w-sm space-y-2 text-sm\">
                                <div class=\"flex justify-between text-gray-500\"><span>Subtotal</span><span>₹${Number(order.subtotal || 0).toFixed(2)}</span></div>
                                <div class=\"flex justify-between text-gray-500\"><span>GST (10%)</span><span>₹${Number(order.tax || 0).toFixed(2)}</span></div>
                                <div class=\"flex justify-between border-t border-gray-200 pt-2 font-bold text-gray-800\"><span>Total</span><span class=\"text-orange-600\">₹${Number(order.total || 0).toFixed(2)}</span></div>
                            </div>
                        </div>
                    </div>`;
            }).join('');
        ...
        // ============================================================
        // MOBILE CART DRAWER — reuses the original cart/payment elements
        // ============================================================
        function setupMobileCart() {
            if (document.getElementById('mobile-cart-toggle')) return;
            const aside = document.querySelector('body > .flex.flex-1 > aside:last-child');
            if (!aside) return;

            const toggle = document.createElement('button');
            toggle.id = 'mobile-cart-toggle';
            toggle.type = 'button';
            toggle.innerHTML = '<i class="fas fa-shopping-basket"></i><span>Cart</span><b id="mobile-cart-count">0</b><strong id="mobile-cart-total">₹0.00</strong>';
            toggle.onclick = () => {
                aside.classList.toggle('mobile-cart-open');
                document.body.classList.toggle('mobile-cart-is-open', aside.classList.contains('mobile-cart-open'));
                updateMobileCartButton();
            };
            document.body.appendChild(toggle);

            const close = document.createElement('button');
            close.id = 'mobile-cart-close';
            close.type = 'button';
            close.innerHTML = '<i class="fas fa-times"></i>';
            close.setAttribute('aria-label', 'Close cart');
            close.onclick = () => {
                aside.classList.remove('mobile-cart-open');
                document.body.classList.remove('mobile-cart-is-open');
                updateMobileCartButton();
            };
        ...
        function updateMobileCartButton() {
            ...
            const amount = cart.reduce((sum, item) => sum + Number(item.price || 0) * Number(item.qty || 0), 0) * 1.1;
